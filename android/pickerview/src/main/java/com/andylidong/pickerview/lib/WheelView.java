...
public class WheelView extends View {
    ...
    Paint paintFirstOuterText;
    Paint paintSecondOuterText;
    ...
    int textFirstColorOut;
    int textSecondColorOut;
    int textColorCenter;
    int dividerColor;
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textFirstColorOut = getResources().getColor(R.color.pickerview_wheelview_textcolor_first_out);
        textSecondColorOut = getResources().getColor(R.color.pickerview_wheelview_textcolor_second_out);
        ...
        if (attrs != null) {
            ...
            textFirstColorOut = a.getColor(R.styleable.pickerview_pickerview_textColorOut, textFirstColorOut);
            textSecondColorOut = a.getColor(R.styleable.pickerview_pickerview_textColorOut, textSecondColorOut);
    ...
    private void initPaints() {
        paintFirstOuterText = new Paint();
        paintFirstOuterText.setColor(textFirstColorOut);
        paintFirstOuterText.setAntiAlias(true);
        paintFirstOuterText.setTypeface(Typeface.SANS_SERIF);
        paintFirstOuterText.setTextSize(textSize);

        paintSecondOuterText = new Paint();
        paintSecondOuterText.setColor(textSecondColorOut);
        paintSecondOuterText.setAntiAlias(true);
        paintSecondOuterText.setTypeface(Typeface.SANS_SERIF);
        paintSecondOuterText.setTextSize(textSize);

        paintCenterText = new Paint();
        paintCenterText.setColor(textColorCenter);
        paintCenterText.setAntiAlias(true);
        paintCenterText.setTypeface(Typeface.create(Typeface.SANS_SERIF,Typeface.BOLD));
        paintCenterText.setTextSize(textSize);

        paintCenterBg = new Paint();
        paintCenterBg.setColor(Color.WHITE);
    ...
    private void remeasure() {
        ...
        //计算两条横线和控件中间点的Y位置
        firstLineY = (measuredHeight - itemHeight - itemHeight - itemHeight) / 2.0F;
        secondLineY = (measuredHeight - itemHeight) / 2.0F;
        thirdLineY = (measuredHeight + itemHeight) / 2.0F;
        fourthLineY = (measuredHeight + itemHeight + itemHeight + itemHeight) / 2.0F;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // canvas.drawLine(0.0F, thirdLineY, measuredWidth, thirdLineY, paintIndicator);
        // 设置中间的背景颜色
        canvas.drawRect(0.0F, secondLineY, (float)getWidth(), thirdLineY, paintCenterBg);
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintFirstOuterText);
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintSecondOuterText);
                    canvas.restore();
                    ...
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintSecondOuterText);
                    canvas.restore();
                    canvas.save();
                    ...
                    canvas.restore();
                } else if (translateY <= thirdLineY && maxTextHeight + translateY >= thirdLineY) {
                    // 条目经过第三条线
                    canvas.save();
                    canvas.clipRect(0, 0, measuredWidth, thirdLineY - translateY);
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, thirdLineY - translateY, measuredWidth, (int) (itemHeight));
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintSecondOuterText);
                    canvas.restore();
                } else if (translateY <= fourthLineY && maxTextHeight + translateY >= fourthLineY) {
                    // 条目经过第四条线
                    canvas.save();
                    canvas.clipRect(0, 0, measuredWidth, fourthLineY - translateY);
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintSecondOuterText);
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, fourthLineY - translateY, measuredWidth, (int) (itemHeight));
                    canvas.scale(1.0F, (float) Math.sin(radian) * 1F);
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintFirstOuterText);
                    canvas.restore();
                    ...
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintSecondOuterText);
                } else if (translateY >= secondLineY && maxTextHeight + translateY <= thirdLineY) {
                    // 中间条目
                    canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintCenterText);
                ...
                } else if (translateY >= thirdLineY && maxTextHeight + translateY <= fourthLineY) {
                    // 中间条目
                    canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));
                    canvas.drawText(contentText, drawCenterContentStart, maxTextHeight - CENTERCONTENTOFFSET, paintSecondOuterText);
                } else {
    ...
    private void measuredOutContentStart(String content) {
        Rect rect = new Rect();
        paintFirstOuterText.getTextBounds(content, 0, content.length(), rect);
        switch (mGravity) {
            case Gravity.CENTER:
    ...
    public void setLineSpacingMultiplier(float lineSpacingMultiplier) { this.lineSpacingMultiplier = lineSpacingMultiplier; }
