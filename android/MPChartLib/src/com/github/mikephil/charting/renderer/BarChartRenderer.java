...
public class BarChartRenderer extends DataRenderer {
    ...
    public void drawValues(Canvas c) {
        ...
        if (passesCheck()) {
            ...
            final float valueOffsetPlus = Utils.convertDpToPixel(2.0f);
            float posOffset = 0f;
            float negOffset = 0f;
                    ...
                    for (int j = 0; j < (valuePoints.length - 1) * mAnimator.getPhaseX(); j += 2) {
                        ...
                        if (vals == null) {
                            ...
                            drawValue(c, dataSet.getValueFormatter(), entry.getVal(), entry, i, valuePoints[j],
                                    valuePoints[j + 1] + (entry.getVal() >= 0 ? (entry.getVal() > 1 ? posOffset : posOffset - 5.0f) : negOffset), dataSet.getValueTextColor(j / 2));
...
//                                float y = transformed[k + 1] + (vals[k / 2] >= 0 ? posOffset : negOffset);
                                float y = transformed[k + 1] + (vals[k / 2] >= 0 ? (vals[k/2] > 1 ? posOffset : posOffset - 5.0f) : negOffset);
                                ...
                                drawValue(c, dataSet.getValueFormatter(), vals[k / 2], entry, i, x, y - 3, color);
                            }
                        }
                    }
                }
