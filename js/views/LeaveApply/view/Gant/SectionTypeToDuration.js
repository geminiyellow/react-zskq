...
export default class SectionTypeToDuration extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.listeners = [
      ...
      DeviceEventEmitter.addListener('REFRESH_LEAVE_CODE', (eventBody) => {
  ...
  onRefreshLeaveEndDate(isShow, leaveEndDateStr) {
    ...
    this.leaveEndDateItem.onRefreshLeaveEndDateValue(isShow, getYYYYMMDDFormat(leaveEndDateStr));
  }
  ...
  onRefreshLeaveSatrtDate(leaveStartDateStr) {
    ...
    // mport { getYYYYMMDDFormat, getYYYYMMDDhhmmFormat, getHHmmFormat }
    this.leaveStartDateItem.onRefreshLeaveStartDateValue(getYYYYMMDDFormat(leaveStartDateStr));
  }
  ...
  onReFreshLeaveStartTime(startTimeStr) {
    ...
    this.leaveStartTimeItem.onRefreshStartTime(true, false, getHHmmFormat(startTimeStr));
  }

  onReFreshLeaveStartTimeNoArrow(startTimeStr) {
    if (!this.leaveStartTimeItem) {
      return;
    }
    this.leaveStartTimeItem.onFreshStartTimeRightArrowShow(false);
    this.leaveStartTimeItem.onRefreshStartTime(true, true, getHHmmFormat(startTimeStr));
  }
  ...
  onReFreshLeaveEndTime(endTimeStr) {
    ...
    this.leaveEndTimeItem.onRefreshEndTime(true, true, getHHmmFormat(endTimeStr));
  }
  ...
  onReFreshLeaveStartTimeHide(startTimeStr) {
    ...
    this.leaveStartTimeItem.onRefreshStartTime(false, true, getHHmmFormat(startTimeStr));
  }
  ...
  onReFreshLeaveEndTimeHide(endTimeStr) {
    ...
    this.leaveEndTimeItem.onRefreshEndTime(false, true, getHHmmFormat(endTimeStr));
  }
  ...
  onHandleDateTimeShow(dateParams, leaveModalId) {
    ...
    switch (leaveModalId) {
      ...
      case 5:
        ...
        // 隐藏开始时间 结束时间
        this.leaveStartTimeItem.onRefreshStartTime(false, true, getHHmmFormat(dateParams.leaveStartTime));
