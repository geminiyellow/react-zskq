...
const mobilechangeicon = 'mobilechange';
...
export default class MobileInfoChangeView extends PureComponent {
  constructor(...args) {
    super(...args);
    this.state = {
      checked: false,
  ...
  render() {
    const empName = this.state.rowData.formDetail.EmpName;
    const Data = getYYYYMMDDFormat(this.state.rowData.formDetail.ApplyDate);
    return (
      <View style={styles.rowStyle}>
        <View style={styles.rowTitleStyle}>
          {this.getRadioView()}
          <View style={styles.circleStyle}>
            <Image style={{ width: 22, height: 22 }} source={{ uri: mobilechangeicon }} />
          </View>
          <Text numberOfLines={1} lineBreakMode="tail" style={styles.rowTitleTextStyle}>{this.state.rowData.formDetail.ToDoTitle}</Text>
          <Text style={styles.statusStyle}>{Data}</Text>
        </View>
        ...
        <TouchableOpacity style={styles.rowContentStyle} onPress={() => this.ItemClick()}>
          <Text style={styles.rowContentDateStyle}>{I18n.t('mobile.module.verify.mobile.change')}</Text>
        </TouchableOpacity>
      </View>
    );
  }
