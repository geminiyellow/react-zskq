...
export default class Attachment extends PureComponent {
  ...
  onInitialization() {
    ...
    this.attViewUriArr = [];
    // 附件限制信息
    const { OTAttachmentAmount } = !_.isEmpty(
      global.loginResponseData.SysParaInfo,
    )
      ? global.loginResponseData.SysParaInfo
      : attachmentNone;
    if (!_.isEmpty(OTAttachmentAmount)) {
      if (OTAttachmentAmount === attachmentNone) {
        this.isApplyAttachUploadLimit = false;
      }
      if (OTAttachmentAmount === attachmentSingle) {
        this.isApplyAttachUploadLimit = true;
      }
      if (OTAttachmentAmount === attachmentMultiple) {
        this.isApplyAttachUploadLimit = true;
