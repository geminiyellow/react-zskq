...
import { getPreviewShoplists } from '@/common/api';
  ...
  getContentView() {
    ...
    return (
      <View style={{ flexGrow: 1, backgroundColor: '#fff' }}>
        <EmptyView emptyimg={nodata} emptyTitle={'这里将展示您管理的组织'} emptyContent={'暂无组织信息'} />
      </View>
    );
  }

  sendRequest() {
    GET(getPreviewShoplists(), (responseData) => {
      ...
      <TouchableOpacity style={{ paddingLeft: 18, backgroundColor: '#fff' }} key={`${sectionID}-${rowID}`} onPress={() => this.ItemClick(rowData)}>
        <View style={{ flexDirection: 'row', paddingVertical: 11 }}>
          ...
          <View style={{ flex: 1, flexDirection: 'column' }}>
            <Text style={{ color: '#333333', fontSize: 14 }}>{rowData.UnitName}</Text>
