...
const processDataSetsColors = (config) => {
  ...
  if ('dataSets' in ret) {
    ret.dataSets = ret.dataSets.map((set) => {
      if ('colors' in set) {
...
export const processColors = (props) => {
  ...
  if ('infoDescriptionTextColor' in config) {
    config.infoDescriptionTextColor = processColor(config.infoDescriptionTextColor);
  }
  ...
  if ('xAxis' in config && 'limitLines' in config.xAxis) {
    config.xAxis.limitLines = config.xAxis.limitLines.map((l) => {
      if ('lineColor' in l) {
  ...
  if ('leftAxis' in config && 'zeroLineColor' in config.leftAxis) {
    config.leftAxis.zeroLineColor = processColor(config.leftAxis.zeroLineColor);
  }
  ...
  if ('leftAxis' in config && 'limitLines' in config.leftAxis) {
    config.leftAxis.limitLines = config.leftAxis.limitLines.map((l) => {
      if ('lineColor' in l) {
        l.lineColor = processColor(l.lineColor);
      }
      if ('valueTextColor' in l) {
  ...
  if ('rightAxis' in config && 'limitLines' in config.rightAxis) {
    config.rightAxis.limitLines = config.rightAxis.limitLines.map((l) => {
      if ('lineColor' in l) {
        l.lineColor = processColor(l.lineColor);
      }
      if ('valueTextColor' in l) {
