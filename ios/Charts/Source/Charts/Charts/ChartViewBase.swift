...
open class ChartViewBase: NSUIView, ChartDataProvider, AnimatorDelegate
{
	...
	/// 是否绘制marker view
	open var isDrawMarkerView: Bool = true
	
	/// 是否是第一次加载完成折线图后默认显示的点
	open var defaultHighlighted: Bool = false
        ...
        else
        {
            ...
            else
            {
				...
				if h!.isEqual(lastHighlighted) {
					isDrawMarkerView = !isDrawMarkerView
				} else {
					isDrawMarkerView = true
				}
            }
        }
    ...
    internal func drawMarkers(context: CGContext)
    {
        ...
        for i in 0 ..< _indicesToHighlight.count
        {
			...
			if set.colors.count > 0 {
				highlight.highlightColor = set.color(atIndex: 0)
			}
			...
			if highlight.isEqual(lastHighlighted) && !isDrawMarkerView {
				continue
			}
