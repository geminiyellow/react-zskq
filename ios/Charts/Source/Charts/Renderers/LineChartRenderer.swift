        ...
        { // only one color per dataset
            ...
            if e1 != nil
            {
                ...
                if !firstPoint
                {
					...
					let shadowColor = dataSet.color(atIndex: 0).withAlphaComponent(0.2).cgColor
					context.setShadow(offset: shadowOffset, blur: 5.0, color: shadowColor)
...
//            let circleHoleRadius = dataSet.circleHoleRadius
			// 内圆半径固定为2.0
			let circleHoleRadius = CGFloat(2.0)
            ...
            for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
            {
                ...
                if drawTransparentCircleHole
                {
					...
					// 绘制阴影
					let shadowOffset = CGSize(width: 0, height: 2)
					let shadowColor = dataSet.color(atIndex: 0).withAlphaComponent(0.4).cgColor
	...
	/// 绘制带实心的圆
	/// 当第一次点击时变为实心圆，再次点击变空心圆
	open override func drawCircles(context: CGContext, indices: [Highlight], defaultHighlighted: Bool)
	{
		guard
			let dataProvider = dataProvider,
			let lineData = dataProvider.lineData,
		...
		for i in 0 ..< dataSets.count
		{
			...
			for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
			{
				...
				else
				{
					...
					if let hightlight = highlight {
						if i == hightlight.dataSetIndex && j == Int(hightlight.x) {
							isHighlight = true
						}
